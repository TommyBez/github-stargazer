...
- **`GITHUB_TOKEN` (or `GITHUB_TOKEN_2`) is optional.** Without it, the `/api/stars`
  route uses the unauthenticated GitHub API, which works but has a low rate limit
  (~60 req/hr per IP). Set one of those env vars to raise the limit if you hit 403/429
  while testing. See `lib/github.ts`.
